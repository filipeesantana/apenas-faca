...
import { DAY, today, addDays, parseISODate, endOfWeek } from '../utils/dates.js';
...
export function buildDemoData() {
  ...
  function task({ title, area, created, due = null, importance = 'normal', postpones = [], started = null, completed = null, dropped = null, goalId = null, notes = '' }) {
    ...
    ev('task.created', t, { dueDate: dueNow, source: 'direct' }, createdAt);
    ...
    if (completed != null) {
      ...
      ev('task.completed', t, { wasOverdue: false, postponedCount: t.postponedCount, dueDate: t.dueDate, importance }, t.completedAt);
    }
  ...
  task({ title: 'Pagar cartão', area: 'Dinheiro', created: 3, due: addDays(T, 1), importance: 'high' });
  task({ title: 'Estudar Python — módulo de funções', area: 'Estudos', created: 20, goalId: python.id,
  ...
  task({ title: 'Responder e-mail do orientador', area: 'Estudos', created: 2, due: addDays(T, -1) });
  task({ title: 'Marcar dentista', area: 'Saúde', created: 25 });
  ...
  task({ title: 'Trocar lâmpada da cozinha', area: 'Casa', created: 10 });
  ...
  for (const [title, area, created, completed] of done) {
    task({ title, area, created, completed, importance: title.includes('proposta') || title.includes('prova') ? 'high' : 'normal' });
  }
