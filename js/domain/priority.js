...
export function scoreTask(t, ctx = context()) {
  ...
  if (t.importance === 'high') add(20, 30, 'tem importância alta');
  else if (t.importance === 'low') add(-10, 0, null);
  ...
  if (goal?.status === 'active') {
    if (t.nextStep) add(10, 22, `é o próximo passo da meta “${goal.title}”`);
    else add(6, 15, `faz parte da meta “${goal.title}”`);
  }
...
/** "Vence hoje e já foi adiada duas vezes." */
export function explain(ranked) {
  const [r0, r1] = ranked.reasons;
  if (!r0) return 'Nada está urgente. É uma boa próxima tarefa.';
  return `${r0.charAt(0).toUpperCase()}${r0.slice(1)}${r1 ? ` e ${r1}` : ''}.`;
}

/**
 * Tarefas que cabem num tempo livre: duração estimada até `minutes`,
 * ordenadas pela prioridade. Ajuda a responder "tenho 30 minutos, o que faço?".
 */
export function fitsIn(minutes, limit = 3) {
  return rankTasks(openTasks().filter((t) => t.estimateMin && t.estimateMin <= minutes)).slice(0, limit);
}
