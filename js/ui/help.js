...
export const HELP = {
  ...
  /* ---------- Camada financeira ---------- */
  disponivel: { title: 'Valor disponível', short: 'O que sobra da renda registrada no período.', text: 'O que resta da renda registrada depois dos gastos e do que foi destinado a metas neste período. Considera apenas o que você registrou — não é um saldo bancário.' },
  natureza: { title: 'Natureza do gasto', short: 'Como você considera este gasto.', text: 'Como você considera o gasto: necessário, importante, flexível ou pontual. É a sua leitura, não uma classificação do Norte. O mesmo valor pode ser essencial para uma pessoa e opcional para outra.', example: 'R$ 500 de transporte podem ser trabalho, consulta médica ou lazer.' },
  flexibilidade: { title: 'Margem para ajuste', short: 'Quanto você aceitaria mexer neste gasto.', text: 'Indica quanto você estaria disposto a alterar este gasto em uma simulação. O Norte não muda nada automaticamente e nunca sugere cortar o que você marcou como “não mexer”.' },
  cenario: { title: 'Cenário', short: 'Uma simulação: não altera seus dados.', text: 'Um cenário é uma simulação. Mexer nos valores aqui não altera saldo, histórico nem a meta. Só se você escolher “usar como plano” o Norte guarda alguma coisa.' },
  planoMeta: { title: 'Plano da meta', short: 'Quanto você decidiu destinar por mês.', text: 'O valor que você decidiu destinar a esta meta por mês — fixo ou como percentual da renda registrada. O Norte compara esse valor com o que foi realmente registrado no mês.' },
  planejadoReal: { title: 'Previsto × realizado', short: 'Previsão não é dinheiro que já entrou ou saiu.', text: 'Entradas e saídas recorrentes são previsões do mês. Elas só contam como realizadas quando você confirma ou registra a movimentação — assim nada é contado duas vezes.' },
  percentualRenda: { title: 'Percentual da renda', short: 'Quanto do seu mês isso representa.', text: 'Mostra quanto um valor representa da renda registrada. É só perspectiva: o Norte não define quanto você “deveria” destinar a cada coisa.' },
};
